...
$('#comment .trumbowyg-editor').on(changeEvent, function(e) {
    var txt =$('#comment .trumbowyg-editor').html();
    setTextWarn.call($('#comment .trumbowyg-editor')[0],txt,150);
});

function showComment(names, l, w) {
    var com = $('#comment');
    $('#commentText').trumbowyg('html', '');
    $('#comtip').html('');
    $('#overlay').show();
    if (names.length) {
        com.find('p').html('回复 | ' + names.first().html());
    } else {
        com.find('p').html('评论原文');
    }
    com.css({ left: l, width: w });
    com.show();

    setTimeout(function() {
        $('#comment').find('.trumbowyg-editor').focus();
    }, 300);
}
//评论提交事件
$('#replyComment').on('click', function(e) {
    ...
    var txt = $('#commentText').trumbowyg('html');
    var tip = $('#comtip');
    if (!txt) {
        tip.html('内容不能为空');
        return;
...
function getShareOpts() {
    ...
    return {
        ...
        sDesc: article.find('.content,.text').html().replace(/\#/g, '').replace(/\<[\s\S]+?\>/g, '').substr(0, 150)
    };
}
...
function uploadFile(e, pic) {
    if (!pic && $('#editImgs').find('img').length >= 3) {
        showDialog({ txt: '抱歉,最多只能上传3张图片', confirm: '确认' });
        return;
    }
